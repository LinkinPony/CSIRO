...
> This document is a **paper / technical report style write-up skeleton** for this repository.
> It is intentionally structured as an outline with prompts, figure/table suggestions, and **repo anchors** you can cite.

## Abstract

This solution is driven by a simple premise: as visual foundation models like DINOv2/DINOv3 mature, many downstream problems can be addressed by **reusing a strong, general-purpose backbone** and learning only a **lightweight task head** (optionally with PEFT/adapters). @simeoniDINOv32025

Concretely, we treat this biomass regression task as “representation + readout”: extract DINOv3 features (CLS and patch tokens) and train a small regression head on top. We also use PEFT methods like LoRA to finetune the DINOv3 backbone. For final submission, we reached 0.78/rank8 (public leaderboard), 0.65/rank6 (private leaderboard) result. For best submission, we reached 0.76/0.69 score on public and private leaderboard.
...
To further improve generalization on a small, noisy dataset, we also apply **manifold mixup** in feature space: during training, we linearly interpolate DINOv3 representations (patch tokens in our final run) between samples and mix supervision targets in a mask-aware way (§3.5.1).

**Suggested figure**

- **Figure 1**: end-to-end pipeline diagram (backbone → tokens → head → predictions → postprocess → submission).

### 1.2 Contributions

Concretely, this repository contributes:

- **A DINOv3 feature-extraction backbone with reproducible loading**: the backbone can be resolved from a vendored local copy (for offline runs) or via `torch.hub`, and supports extracting both last-layer and intermediate-layer features for multi-layer heads. See [`../src/models/backbone.py`](../src/models/backbone.py).
- **Optional PEFT/LoRA finetuning for ViT backbones**: LoRA adapters are injected into selected attention/MLP projections (typically in the last \(K\) blocks), with a “train adapters only” policy and optional EVA initialization support. See [`../src/models/peft_integration.py`](../src/models/peft_integration.py).
- **A unified regression model with multiple lightweight heads**: a single training stack supports different readout heads (MLP / ViTDet-style pyramid / FPN / Mamba / DPT / EoMT), patch-aware modes, and multi-layer feature fusion, enabling systematic ablations under consistent data and optimization settings. See [`../src/models/regressor/biomass_regressor.py`](../src/models/regressor/biomass_regressor.py) and `src/models/*_head.py`.
- **Feature-space augmentation via manifold mixup**: we apply mixup directly on the shared representation (patch tokens in our final run) and mix regression/ratio targets consistently (including missing-label masks), providing a strong regularizer without pixel-level mixing. See [`../src/models/regressor/manifold_mixup.py`](../src/models/regressor/manifold_mixup.py).
- **Practical weights management for deployment**: backbone weights and head weights are treated as separate artifacts (head-only checkpoints and packaging), enabling fast swapping and ensembling without duplicating large backbone checkpoints. See [`../src/callbacks/head_checkpoint.py`](../src/callbacks/head_checkpoint.py), [`../package_artifacts.py`](../package_artifacts.py), and [`../README.md`](../README.md).
- **An end-to-end inference pipeline for competition submission**: inference loads backbone + head weights, supports test-time augmentation with ViT patch-grid alignment, optional ensembling (configured in JSON), and writes the required long-format `submission.csv`. See [`../infer_and_submit_pt.py`](../infer_and_submit_pt.py), [`../src/inference/pipeline.py`](../src/inference/pipeline.py), and [`../configs/ensemble.json`](../configs/ensemble.json).

**Repo anchors**

- Backbone loader / feature extractor: [`../src/models/backbone.py`](../src/models/backbone.py)
- LoRA integration: [`../src/models/peft_integration.py`](../src/models/peft_integration.py)
- Model entry (head types & knobs): [`../src/models/regressor/biomass_regressor.py`](../src/models/regressor/biomass_regressor.py)
- Manifold mixup: [`../src/models/regressor/manifold_mixup.py`](../src/models/regressor/manifold_mixup.py)
- Inference pipeline: [`../infer_and_submit_pt.py`](../infer_and_submit_pt.py), [`../src/inference/pipeline.py`](../src/inference/pipeline.py)
...
`Height_Ave_cm` / `Pre_GSHH_NDVI` / `State` are used as auxiliary targets when enabled (§3.5).
...
capture-condition variation** while keeping the overall pasture layout intact. Concretely, `configs/train.yaml` enables:
...
In `configs/train.yaml`, `model.log_scale_targets: false`, so no log transform is applied to training labels. The
...
**global-baseline** variant where \(\overline{\tilde{y}}_k\) is taken from the full dataset (train + validation targets
for all folds), rather than from one fold only.

## 3. Method

### 3.1 Overall pipeline [TODO]

Include a high-level diagram and a concise description of the “representation + readout” framing.

### 3.2 Backbone: DINOv3 features

This repository uses **DINOv3** as the visual backbone. Conceptually, we treat DINOv3 as a general-purpose image encoder that converts an input pasture image into a set of dense visual features; a lightweight regression head then maps those features to biomass targets (§3.4).

In our final configuration, we use **DINOv3 ViT-7B/16** (`dinov3_vit7b16`) pretrained on LVD-1689M, and we use it primarily as a **feature extractor** for downstream regression (with optional lightweight adaptation via LoRA; §3.3).

#### 3.2.1 Patch tokens as a spatial representation
...
For the DINOv3 ViT-7B/16 backbone, \(C=4096\). At our typical resolution (960×480), \(H_p=60\), \(W_p=30\), so \(N=1800\) patch tokens (data augmentation such as random resized crops can change the exact grid at training time).

We focus on **patch tokens** rather than the pooled **CLS token**, because our readout is spatial: it reshapes tokens back into a 2D feature map and builds multi-scale features to support robust scalar prediction (§3.4). Patch tokens preserve locality (e.g., separating vegetation from background) while still encoding global context through self-attention.

#### 3.2.2 Multi-layer feature extraction and fusion
...
To combine multiple layers, we first align their feature spaces with lightweight per-layer projections (bottlenecks), then apply a **learned fusion** to produce a single fused token representation passed to the downstream head. (When using multi-layer heads, we may also run a separate head per selected layer and fuse their predictions; see §3.4.4.)

**Repo anchors**

- Training config: [`../configs/train.yaml`](../configs/train.yaml)
- DINOv3 wrapper and intermediate-layer extraction: [`../src/models/backbone.py`](../src/models/backbone.py)
- Layer fusion utilities: [`../src/models/layer_utils.py`](../src/models/layer_utils.py)

### 3.3 Backbone adaptation: frozen backbone + LoRA (optional)

The CSIRO biomass dataset is small compared to the capacity of a multi-billion-parameter ViT backbone, so we avoid full end-to-end finetuning by default. Instead, we train in a **“frozen backbone + lightweight readout”** regime: DINOv3 provides stable patch-token features (§3.2), and nearly all task learning happens in the head (§3.4).
...
#### 3.4.1 Per-layer readout: patch tokens → pyramid → global descriptor
...
#### 3.4.2 Predicting total + ratio (and auxiliary scalars)

From the shared bottleneck \(\mathbf{z}\), we predict:

- **Main scalar (`reg3`)**: in the final config, `data.target_order: [Dry_Total_g]`, so `reg3` predicts a single **total** term \(t\). With `use_output_softplus: true`, we apply a Softplus to encourage \(t \ge 0\).
...
#### 3.4.3 Multi-layer inference: per-layer heads → learned fusion → 5D biomass

The final config enables multi-layer extraction (`model.backbone_layers.enabled: true`) from DINOv3 layers **[23, 31, 39]**. We apply the same per-layer readout described above (§3.4.1–§3.4.2) to each layer’s patch tokens, producing per-layer predictions (total \(t_\ell\), ratio logits \(r_\ell\), and bottleneck \(\mathbf{z}_\ell\)).
...
### 3.5 Training objective and multi-task optimization
...
This section covers: (i) feature-space regularization via manifold mixup (§3.5.1), (ii) the loss terms (§3.5.2), (iii) how we balance them (UW; §3.5.3), and (iv) how we mitigate gradient interference (PCGrad; §3.5.4).

#### 3.5.1 Feature-space augmentation: manifold mixup
...
In `configs/train.yaml`, manifold mixup is enabled with high probability (≈0.96) and a Beta parameter \(\alpha\approx 3.9\) (strong mixing). It is applied during training only and disabled for validation/inference.

#### 3.5.2 Loss terms (what we optimize)
...
Below we define each raw task loss \(L_t\) before describing how we combine them (§3.5.3) and how we handle gradient interference (§3.5.4).
...
In `configs/train.yaml`, we set the weight vector \(w\) to match the leaderboard emphasis:
\[
...
#### 3.5.3 Uncertainty Weighting (UW) for loss balancing

With multiple loss terms, a naïve sum requires hand-tuned coefficients (and those coefficients can change when we add/remove tasks, change normalization, or change batch composition). In `configs/train.yaml` we enable **Uncertainty Weighting (UW)** via `loss.weighting: uw` and learn the balancing automatically.
...
Given the raw task losses \(L_t\) from §3.5.2, the total scalar loss is:
...
In implementation, these UW parameters are stored as a `ParameterDict` keyed by task name and are optimized as their own optimizer group (controlled by `optimizer.uw_lr` and `optimizer.uw_weight_decay`). See the source-of-truth hooks in [`../src/models/regressor/steps/uw.py`](../src/models/regressor/steps/uw.py) and UW parameter creation in [`../src/models/regressor/biomass_regressor.py`](../src/models/regressor/biomass_regressor.py).

#### 3.5.4 PCGrad (primary-anchored) for gradient conflict mitigation
...
The source-of-truth implementation lives in [`../src/models/regressor/biomass_regressor.py`](../src/models/regressor/biomass_regressor.py) (custom `backward` hook) and [`../src/training/pcgrad.py`](../src/training/pcgrad.py) (projection routines).
...
Run the legacy single-file YAML configuration:
...
- Legacy launcher: [`../train.py`](../train.py)
- Training entrypoint: [`../src/training/entrypoint.py`](../src/training/entrypoint.py)
- Legacy YAML training routine: [`../src/training/single_run.py`](../src/training/single_run.py)
...
- Core LightningModule: [`../src/models/regressor/biomass_regressor.py`](../src/models/regressor/biomass_regressor.py)
- Training runner: [`../src/training/single_run.py`](../src/training/single_run.py)
...
- **Packaged inference folder**: `package_artifacts.py` copies a selected head checkpoint to `weights/head/infer_head.pt`
...
- Head checkpointing callback: [`../src/callbacks/head_checkpoint.py`](../src/callbacks/head_checkpoint.py)
- Lightning checkpoint slimming: [`../src/models/regressor/checkpointing.py`](../src/models/regressor/checkpointing.py)
- Packaging script: [`../package_artifacts.py`](../package_artifacts.py)
- Inference loader for head weights: [`../src/inference/torch_load.py`](../src/inference/torch_load.py)
...
multi-model ensemble defined in `configs/ensemble.json`.

### 5.1 Entrypoint (`infer_and_submit_pt.py`)

Inference runs fully offline via [`../infer_and_submit_pt.py`](../infer_and_submit_pt.py). Set the required variables at
...
target parameterization). In the default setup this is `configs/train.yaml`, and in ensemble mode each member may use a
per-version snapshot (see §5.2.2).

### 5.2 The submission ensemble we used (`configs/ensemble.json`)

Our final submission uses a **4-model ensemble** configured in [`../configs/ensemble.json`](../configs/ensemble.json):
...
[`../package_artifacts.py`](../package_artifacts.py) (see §4.3).
...
- Submission script: [`../infer_and_submit_pt.py`](../infer_and_submit_pt.py)
- Core inference pipeline: [`../src/inference/pipeline.py`](../src/inference/pipeline.py)
- Ensemble utilities + cache IO: [`../src/inference/ensemble.py`](../src/inference/ensemble.py)
- Ensemble config: [`../configs/ensemble.json`](../configs/ensemble.json)
- Packaging: [`../package_artifacts.py`](../package_artifacts.py)
...
- compute (GPU model / RAM), training time, batch size, precision, seeds
- image resolution(s)
- dataset split protocol

### 6.2 Baselines and ablations (recommended)

Suggested ablations (keep only what you actually ran):

- pooled CLS vs mean(patch) vs CLS+patch
- frozen backbone vs LoRA
- head type comparison (MLP/VitDet/FPN/Mamba/DPT/EoMT)
- last-layer vs multi-layer feature fusion
- EMA/SWA, TTA, ensemble gains
- loss-space alignment (`legacy` vs `kaggle_log1p_grams`)

**Suggested tables**

- **Table 3**: ablation study (offline metric and/or LB)
- **Table 4**: final model / ensemble composition

## 7. Results

- **Leaderboard results**: final submission and best submission scores (public/private) + ranks.
- **Offline vs LB**: discuss correlation and sources of mismatch.
- **Per-target breakdown** (optional): R² or error per target, and qualitative failure examples.

## 8. Discussion

Suggested talking points:

- why the approach works (representation strength + head inductive bias)
- where it fails (domain shift, mixed species, lighting, label noise)
- engineering trade-offs (head-only portability vs ensemble complexity)

## 9. Limitations and Future Work

- stronger spatial supervision (e.g., dense NDVI / patch-level constraints)
- more robust splits and calibration
- external data / self-supervised adaptation

**Repo anchors**

- Spatial modeling notes: [`fpn_spatial_model_plan.md`](fpn_spatial_model_plan.md)

## 10. Conclusion

One paragraph:

- summarize framing, key engineering decisions, and achieved leaderboard performance.

## References

- DINOv3 / DINOv2 papers
- LoRA / DoRA / PEFT refs
- (Optional) related work on ViT feature reuse for regression / dense prediction

## Appendix (optional)

### A. Reproducibility checklist

- exact configs / overrides for your final model(s)
- exact weights used and how they were packaged

**Repo anchors**

- Training commands and Ray cluster notes: [`../README_TRAINING.md`](../README_TRAINING.md)
- Inference script variables: [`../infer_and_submit_pt.py`](../infer_and_submit_pt.py)