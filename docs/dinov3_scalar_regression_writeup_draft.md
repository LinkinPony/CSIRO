...
- **Composition ratio** $p$: a 3-way proportion vector over **(clover, dead, green)**. In the dataset, when
...
`Pre_GSHH_NDVI`). The `(Sampling_Date, State)` pair is used for grouped cross-validation (§2.3), and
...
All biomass targets are reported in grams for a fixed plot size of $0.7\,\mathrm{m} \times 0.3\,\mathrm{m}$. We convert grams
...
- **Fold construction**: we build folds on the image-level dataframe and (by default) group by
  `(Sampling_Date, State)`, i.e. all images sharing the same date–state pair are assigned to the same
...
$X$ (shape $B \times 4096 \times H_p \times W_p$), where $(H_p, W_p)$ is the patch grid implied by the input image size.
...
$\{2.0, 1.0, 0.5, 0.25\}$ (relative to the base grid):
   ...
   Each level is then projected to a common width of **512 channels** via lightweight conv blocks ($1\times1 \to \mathrm{LN} \to 3\times3 \to \mathrm{LN}$), matching the ViTDet convention of channel-wise LayerNorm in NCHW.
...
4) **Applies an MLP to obtain the shared bottleneck.** We pass $g$ through a small scalar MLP ($2048 \to 4096$ with **SiLU** and strong dropout) to produce
$z$ in $\mathbb{R}^{4096}$. This $z$ is the shared representation used by all scalar prediction heads.
...
Concretely, when 5D component masses are available $y_i^{(5)} = [c_i, d_i, g_i, \mathrm{GDM}_i, t_i]$, we **mix masses first** and derive ratio targets from the mixed masses (with a small epsilon for numerical safety):

$$
\mathbf{y}^{(5)\prime}_i = \lambda \mathbf{y}^{(5)}_i + (1-\lambda) \mathbf{y}^{(5)}_{\pi(i)}
$$
...
- ratio logits $\hat{\mathbf{r}}_i \in \mathbb{R}^3$ for (clover, dead, green), with probabilities $\hat{\mathbf{p}}_i = \mathrm{softmax}(\hat{\mathbf{r}}_i)$,
...
$$
\begin{aligned}
\hat{y}^{(5)}_{i,\text{clover}} &= \hat{p}_{i,0}\hat{t}_i, \\
\hat{y}^{(5)}_{i,\text{dead}} &= \hat{p}_{i,1}\hat{t}_i, \\
...
\begin{aligned}
\hat{y}^{(5)}_{i,\text{gdm}}
&= \hat{y}^{(5)}_{i,\text{clover}} + \hat{y}^{(5)}_{i,\text{green}}, \\
\hat{y}^{(5)}_{i,\text{total}}
&= \hat{y}^{(5)}_{i,\text{clover}} + \hat{y}^{(5)}_{i,\text{dead}} + \hat{y}^{(5)}_{i,\text{green}}.
\end{aligned}
$$
...
L_{\text{reg3}}
&=
\frac{1}{\sum_{i\in\mathcal{B}} m^{(\text{reg3})}_i} \quad \sum_{i\in\mathcal{B}} m^{(\text{reg3})}_i \,\big(\hat{t}_i - t_i\big)^2.
\end{aligned}
$$
...
L_{\text{ratio}}
&=
\frac{1}{\sum_{i\in\mathcal{B}} m^{(\text{ratio})}_i} \quad \sum_{i\in\mathcal{B}} m^{(\text{ratio})}_i \,\left\lVert \hat{\mathbf{p}}_i - \mathbf{p}_i \right\rVert_2^2.
\end{aligned}
$$
...
- state: a multi-class cross-entropy $L_{\text{state}} = \mathrm{CE}(\hat{s}_i, s_i)$.
...
We maintain one learnable scalar **log-variance** per task, $s_t = \log(\sigma_t^2)$, for all enabled tasks $\mathcal{T}=\{\mathrm{reg3}, \mathrm{ratio}, \mathrm{biomass\_5d}, \mathrm{height}, \mathrm{ndvi}, \mathrm{state}\}$.
Given the raw task losses $L_t$ from §3.4.2, the total scalar loss is:
$$
\begin{aligned}
L
&=
\frac{1}{|\mathcal{T}|}\sum_{t\in\mathcal{T}} \quad \frac{1}{2}\left(\exp(-s_t)\,L_t + s_t\right).
\end{aligned}
$$
...
\begin{aligned}
g_a
&\leftarrow g_a - \frac{\langle g_a, g_p\rangle}{\lVert g_p\rVert^2 + \varepsilon}\, g_p \quad\text{if}\quad \langle g_a, g_p\rangle < 0.
\end{aligned}
$$
...
PCGrad operates on per-task **terms that sum to the exact scalar loss used for backprop**, not on the raw $L_t$. Concretely, when UW is enabled, each task term becomes $0.5 \cdot \big(\exp(-s_t)\,L_t + s_t\big)$ (and we keep the same normalization by number of tasks). This makes the PCGrad procedure consistent with the true UW objective, rather than a separate reweighted surrogate.
...
- `third_party/DI-Retinex` ([sunshangquan/DI-Retinex](https://github.com/sunshangquan/DI-Retinex))

- `third_party/DPT` ([isl-org/DPT](https://github.com/isl-org/DPT))

- `third_party/F-SAM` ([nblt/F-SAM](https://github.com/nblt/F-SAM))

- `third_party/GSAM` ([juntang-zhuang/GSAM](https://github.com/juntang-zhuang/GSAM))

- `third_party/LibMTL` ([median-research-group/LibMTL](https://github.com/median-research-group/LibMTL))

- `third_party/TabPFN` ([PriorLabs/TabPFN](https://github.com/PriorLabs/TabPFN))

- `third_party/albumentations` ([albumentations-team/albumentations](https://github.com/albumentations-team/albumentations))

- `third_party/augmix` ([google-research/augmix](https://github.com/google-research/augmix))

- `third_party/c-mixup` ([huaxiuyao/C-Mixup](https://github.com/huaxiuyao/C-Mixup))

- `third_party/causal-conv1d` ([Dao-AILab/causal-conv1d](https://github.com/Dao-AILab/causal-conv1d))

- `third_party/d2` ([terrastruct/d2](https://github.com/terrastruct/d2))

- `third_party/detectron2` ([facebookresearch/detectron2](https://github.com/facebookresearch/detectron2))

- `third_party/dinov3` ([facebookresearch/dinov3](https://github.com/facebookresearch/dinov3))

- `third_party/dinov3-finetune` ([RobvanGastel/dinov3-finetune](https://github.com/RobvanGastel/dinov3-finetune))

- `third_party/dinov3-src` (source snapshot of DINOv3; upstream: [facebookresearch/dinov3](https://github.com/facebookresearch/dinov3))

- `third_party/eomt` ([tue-mps/eomt](https://github.com/tue-mps/eomt))

- `third_party/fixmatch` ([google-research/fixmatch](https://github.com/google-research/fixmatch))

- `third_party/lightly-train` ([lightly-ai/lightly-train](https://github.com/lightly-ai/lightly-train))

- `third_party/mamba` ([state-spaces/mamba](https://github.com/state-spaces/mamba))

- `third_party/multi_task_learning_pytorch` ([SimonVandenhende/Multi-Task-Learning-PyTorch](https://github.com/SimonVandenhende/Multi-Task-Learning-PyTorch))

- `third_party/peft` ([huggingface/peft](https://github.com/huggingface/peft))

- `third_party/peft-src` (source snapshot of PEFT; upstream: [huggingface/peft](https://github.com/huggingface/peft))

- `third_party/pytorch-lightning` ([Lightning-AI/pytorch-lightning](https://github.com/Lightning-AI/pytorch-lightning))

- `third_party/ray` ([ray-project/ray](https://github.com/ray-project/ray))

- `third_party/sam` ([davda54/sam](https://github.com/davda54/sam))

- `third_party/uda` ([google-research/uda](https://github.com/google-research/uda))

## 10. Papers

- **DINOv3** — Oriane Siméoni et al., 2025. [arXiv:2508.10104](https://arxiv.org/abs/2508.10104)

- **Gradient Surgery for Multi-Task Learning (PCGrad)** — Tianhe Yu et al., 2020. [arXiv:2001.06782](https://arxiv.org/abs/2001.06782)

- **Multi-Task Learning Using Uncertainty to Weigh Losses for Scene Geometry and Semantics** — Alex Kendall, Yarin Gal, Roberto Cipolla, 2018. [arXiv:1705.07115](https://arxiv.org/abs/1705.07115)

- **LoRA: Low-Rank Adaptation of Large Language Models** — Edward J. Hu et al., 2021. [arXiv:2106.09685](https://arxiv.org/abs/2106.09685)

- **Exploring Plain Vision Transformer Backbones for Object Detection (ViTDet)** — Yanghao Li et al., 2022. [arXiv:2203.16527](https://arxiv.org/abs/2203.16527)